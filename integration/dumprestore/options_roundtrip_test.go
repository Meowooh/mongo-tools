...
func (s *DumpRestoreSuite) testCappedRoundTripSingleColl() {
	...
	s.Assert().EqualValues(
		fixture.docCount,
		s.docCount(restoredColl),
		"all capped documents are restored into the renamed collection",
	)
	s.Assert().Len(
		s.indexNames(restoredColl),
		2,
		"the renamed collection has its _id and secondary index",
	)
	s.assertCappedEvictionWorks(restoredColl, fixture.docCount)
}
...
func (s *DumpRestoreSuite) createCappedFixture(testDB *mongo.Database) cappedFixture {
	...
	options := s.collectionOptions(testDB, cappedColl.Name())
	s.Require().
		Equal(true, optionValue(options, "capped"), "the fixture collection really is capped")
	...
	return cappedFixture{docCount: cappedCount, options: options}
}

func (s *DumpRestoreSuite) createCollection(
	testDB *mongo.Database,
	collName string,
	opts *mopt.CreateCollectionOptionsBuilder,
) *mongo.Collection {
...
func (s *DumpRestoreSuite) assertCappedFixtureRestored(
	...
	s.Assert().EqualValues(
		fixture.docCount,
		s.docCount(cappedColl),
		"the capped collection is restored with all of its documents",
	)
	...
	s.Assert().ElementsMatch(
		[]string{"_id_", "x_1"},
		s.indexNames(cappedColl),
		"the capped collection's indexes are restored",
	)
	...
	s.assertCappedEvictionWorks(cappedColl, fixture.docCount)
}
...
// field. Modern mongodump cannot produce such a dump, so the metadata is a
// hand-written string rather than marshaled from Go values.
func (s *DumpRestoreSuite) TestRestoreExtendedJSONMetadata() {
	...
	metadata := fmt.Sprintf(
		`{"options":{"size":{"$numberLong":"10000000"},"capped":true},`+
			`"indexes":[`+
			`{"v":1,"key":{"_id":{"$numberLong":"1"}},"ns":%q,"name":"_id_"},`+
			`{"v":1,"key":{"pos":"2d"},"ns":%q,"name":"position_2d",`+
			`"min":{"$numberLong":"0"},"max":{"$numberLong":"1000"},`+
			`"bits":{"$numberLong":"32"}}]}`,
		ns,
		ns,
	)
	s.Require().NoError(
		os.WriteFile(filepath.Join(dbDir, collName+".metadata.json"), []byte(metadata), 0644),
	...
	options := s.collectionOptions(testDB, collName)
	s.Assert().Equal(true, optionValue(options, "capped"), "the capped option is restored")
	// The server rounds the capped size to a storage-engine-friendly value.
	s.Assert().InDelta(
		10_000_000,
		optionValue(options, "size"),
		1000,
...
func (s *DumpRestoreSuite) createOptionsFixture(testDB *mongo.Database) {
	s.createCollection(
		testDB,
		"capped",
		mopt.CreateCollection().SetCapped(true).SetSizeInBytes(optionsFixtureCappedSize),
	)
	s.createCollection(
		testDB,
		"validated",
		mopt.CreateCollection().SetValidator(bson.D{{"phone", bson.D{{"$type", "string"}}}}),
	)
...
func (s *DumpRestoreSuite) TestCollationRoundTrip() {
	...
	coll := s.createCollection(
		testDB,
		collName,
		mopt.CreateCollection().SetCollation(&mopt.Collation{Locale: "fr_CA"}),
	)
...
func (s *DumpRestoreSuite) testRestoredValidatorDropsInvalidDocs() {
	...
	s.Assert().NotNil(
		...
		"the validator itself is restored",
	)
}
...
func (s *DumpRestoreSuite) testBypassRestoredValidator() {
	...
	s.Assert().NotNil(
		s.collectionOption(testDB, validationCollName, "validator"),
		"the validator itself is still restored",
	)
}
...
func (s *DumpRestoreSuite) createValidatedCollection(testDB *mongo.Database) {
	s.createCollection(
		testDB,
		validationCollName,
		mopt.CreateCollection().SetValidator(validationFixtureValidator),
	)
}
