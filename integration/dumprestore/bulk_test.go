...
const (
	largeDocSize  = 4 * 1024 * 1024
	largeDocCount = 3
)

// TestRestoreLargeBulk checks that multi-megabyte documents survive a dump and
// restore byte for byte, batched together rather than one per write.
//
// It deliberately does not exercise the size-based batch split that TOOLS-939
// was about. The buffered bulk inserter flushes at MAX_MESSAGE_SIZE_BYTES minus
// 1MB, roughly 47MB, so reaching that threshold would mean dumping and restoring
// more than 47MB on every CI topology and server version. That split is already
// covered by unit tests in common/db/buffered_bulk_test.go, so this asserts only
// what a modest fixture can honestly show.
func (s *DumpRestoreSuite) TestRestoreLargeBulk() {
	...
	s.withBSONMongodump(func(dir string) {
		...
		s.Require().NoError(result.Err, "can restore multi-megabyte documents")
